...
DESCRIPTION
    matlab_to_csv_map(path_matlab_file, path_csv_folder)
        ...
        Trial 59 was different to the rest of the tracks, because it had only one track of 0.13 seconds. So this track is excluded from the dataset.
    filtering_nan(lst)
...
PARAMETERS
    path_matlab_file is the path to the original matlab file (string).
        I used: '/Users/nubia/Desktop/Thesis_2.0/dataset/Database_time.mat', but the user should ut in their own path.
    path_csv_folder is the path to where the new csv folder datastructure should be stored (string)
    ...
    with (h5py.File(path_matlab_file, 'r') as mat_file): # open math file
        ...
        for trial_number in range(trial.shape[0]):
            if trial_number < 58: # to exclude trial 59 trial 60 needs to become trial 59
                ...
                a = 0
            if trial_number != 58:
                print(trial_number+a)
                new_map = os.path.join(basemap_path, f'Trial_{trial_number+a}')
